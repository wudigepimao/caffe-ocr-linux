...
std::wstring string2wstring(const string& str, bool bSrcIsUTF8 = true)
{
	...
	int nRet = ::MultiByteToWideChar(srcCode,
        ...
        wstring wstr = dst;
#else
	//printf("=====str====%s,len=%lu\n", str.c_str(), str.size());i
        wchar_t* dst = NULL;
        if(bSrcIsUTF8){
           // setlocale(LC_ALL, "zh_CN.UTF-8");
            int wsize = mbstowcs(dst,str.c_str(),0)+1;
            dst = new wchar_t[wsize];
            wmemset(dst,0,wsize);
            mbstowcs(dst,str.c_str(),wsize);
        }
	// 	if (wstr.size() == 0)
	...
	const string& strlabel, const std::map<wchar_t, int>& mapLabel2Idx)
{
	...
	{
		std::map<wchar_t, int>::const_iterator it = mapLabel2Idx.find(strlabel[i]);
...
void test_ocr_english(const string& imgfolder, const string& modelfolder, const string& lexiconfile)
{
	...
	std::map<wchar_t, int> mapLabel2IDs;
...
void test_ocr_chinese(const string& imgfolder, const string& modelfolder)
{
	...
	for (size_t i = 0; i < alphabets.size(); i++)
	{
		...
		if (alphabets[i] == "blank")
			continue;
		wstring wlabel = string2wstring(alphabets[i], true);
		mapLabel2IDs.insert(make_pair(alphabets[i][0], i));
	}
...
int main(int argc, char *argv[])
{
        ...
        if(argc<3){
            printf("ocr_test imgfolder modelfolder [chi|eng]\n");
            exit(1);
        }


        char *imgfolder = argv[1];
        char *modelfolder = argv[2];
...
#else
	//string imgfolder = "/home/jiachx/ocrtest/ocr_test";
	//string modelfolder = "/home/jiachx/ocrtest/inception-bn-res-blstm";
#endif
}
